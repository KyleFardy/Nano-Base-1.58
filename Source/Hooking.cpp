...
void Hooking::FindPatterns()
{
	...
	c_location = p_gameState.count(1).get(0).get<char>(7);
	...
	printf("got gameState: 0x%p\n", m_gameState);

	printf("[DEBUG] Got game State \n");
	...
	printf("got native registration: 0x%p\n", m_registrationTable);
	...
	printf("got Blip list: 0x%p\n", m_blipList);

	printf("got gameState: 0x%p\n", m_gameState);
	Log::Msg("Blip List: 0x%p", m_blipList);
	Log::Msg("got vector3 res: 0x%p", p_fixVector3Result);
	Log::Msg("got gameState: 0x%p", m_gameState);
	Log::Msg("got worldpointer : 0x % p", m_worldPtr);

	printf("pmodelCheck : 0x%p\n", p_modelCheck);
	printf("pmodelSpawn : 0x%p\n", p_modelSpawn);
	printf("vLocation: 0x%i\n", v_location);

	Log::Msg("got pmodelcheck : 0x%p", p_modelCheck);
	Log::Msg("got pmodelSpawn: 0x%p", p_modelSpawn);
	Log::Msg("got vLocation : 0x % p", v_location);
	...
	Log::Msg("GTA V ready!");
	printf("Finished, GTA V is now ready. \n");
}
